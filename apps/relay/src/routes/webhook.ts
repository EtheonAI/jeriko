...
import { getConnection, findByTriggerId, sendTo } from "../connections.js";
...
async function extractWebhookPayload(c: { req: { text(): Promise<string>; raw: Request } }): Promise<{
  ...
  return { body, headers, requestId: randomUUID() };
}
...
export function webhookRoutes(): Hono {
  ...
  router.post("/:triggerId", async (c) => {
    ...
    const conn = findByTriggerId(triggerId);
    if (!conn) {
      return c.json({
        ok: false,
    ...
    const sent = sendTo(conn.userId, message);
    if (!sent) {
      return c.json({
        ok: false,
        error: "Failed to forward webhook (connection lost)",
